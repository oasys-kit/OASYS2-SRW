...
from wofrysrw.beamline.optical_elements.gratings.srw_elliptical_grating import SRWEllipticalGrating

from orangecontrib.srw.widgets.gui.ow_srw_grating import OWSRWGrating

class OWSRWEllipticalGrating(OWSRWGrating):

    name = "Elliptical Grating"
    description = "SRW: Elliptical Grating"
    ...
    def get_grating_instance(self):
        return SRWEllipticalGrating(distance_from_first_focus_to_mirror_center=self.distance_from_first_focus_to_mirror_center,
    ...
    def draw_specific_box(self):
        ...
        oasysgui.lineEdit(self.substrate_box, self, "distance_from_first_focus_to_mirror_center", "1st focus to mirror center distance (p) [m]", labelWidth=260, valueType=float, orientation="horizontal")
        oasysgui.lineEdit(self.substrate_box, self, "distance_from_mirror_center_to_second_focus", "Mirror center to 2nd focus distance (q) [m]", labelWidth=260, valueType=float, orientation="horizontal")


    def receive_shape_specific_syned_data(self, optical_element):
        if not isinstance(optical_element.get_surface_shape(), Ellipsoid):
            raise Exception("Syned Data not correct: Grating Surface Shape is not Elliptical")
