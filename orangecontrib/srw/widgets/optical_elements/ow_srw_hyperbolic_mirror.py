...
from syned.beamline.shape import Hyperboloid

from wofrysrw.beamline.optical_elements.mirrors.srw_hyperbolic_mirror import SRWHyperbolicMirror

from orangecontrib.srw.widgets.gui.ow_srw_mirror import OWSRWMirror

class OWSRWHyperbolicMirror(OWSRWMirror):

    name = "Hyperbolic Mirror"
    description = "SRW: Hyperbolic Mirror"
    ...
    def get_mirror_instance(self):
        return SRWHyperbolicMirror(distance_from_first_focus_to_mirror_center=self.distance_from_first_focus_to_mirror_center,
    ...
    def receive_shape_specific_syned_data(self, optical_element):
        if not isinstance(optical_element.get_surface_shape(), Hyperboloid):
            raise Exception("Syned Data not correct: Mirror Surface Shape is not Hyperbolic")
